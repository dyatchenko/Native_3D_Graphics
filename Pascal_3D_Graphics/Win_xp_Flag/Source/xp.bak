...
begin
  ...
  for i:=-50 to 50 do begin
    for id:=-50 to 50 do begin
      ...
      massiv1[h].x:=id*4;
      massiv1[h].z:=i*4;
      massiv1[h].y:=round(100*cos(id*p+pi/2));
      if (id<0)and(i<0) then massiv1[h].c:=blue;
      if (id>0)and(i<0) then massiv1[h].c:=yellow;
      if (id<0)and(i>0) then massiv1[h].c:=red;
      if (id>0)and(i>0) then massiv1[h].c:=green;
      if (id>-5)and(id<5) then massiv1[h].c:=black;
      if (i>-5)and(i<5) then massiv1[h].c:=black;
      massiv1[h].f:=pi/100*id;
    end;
  end;
  ...
  repeat
    ...
    inc(ut,12);
    if ut/7>360 then ut:=0;

    ReadKeyDown;

    for i:=1 to 10000 do
      if i mod 2 = 0 then
        put3dpixel(massiv1[i].x,massiv1[i].y-7,massiv1[i].z,massiv1[i].c)
        else put3dpixel(massiv1[i].x,massiv1[i].y,massiv1[i].z,massiv1[i].c);

    for i:=1 to 10000 do begin
      massiv1[i].y:=round(70*cos(ut*p+massiv1[i].f));
    end;
