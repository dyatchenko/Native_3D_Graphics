...
massiv1,massiv2:array[-100..10000] of dir;
...
begin
  ...
  for i:=-50 to 50 do begin
    ...
    {  massiv1[h].y:=round(100*cos(sqrt( sqr((i-50)*10/50)+sqr((id-50)*10/50) )));
     { massiv[h].y:=round(100*cos((i-50)*4/50)*sin((id-50)*4/50));
{      if (i<>50)and(id<>50) then massiv[h].y:=300/(sqrt(sqr((i-50)*4/50)+sqr((id-50)*4/50)));}
{      if (i<>50)and(id<>50) then massiv[h].y:=300/cos(sqrt(sqr((i-50)*4/50)+sqr((id-50)*4/50)));}
{      massiv[h].y:=abs(i-50)*abs(id-50);}
{      if id<>50 then massiv[h].y:=100*(i-50)/(id-50);  }
{      massiv[h].y:=sqr(i-50)/4+sqr(id-50)/4; }
      if (sqr(i)+sqr(id))<=2500 then massiv1[h].y:=round(sqrt(2500-sqr(i)-sqr(id)))*17
        else begin
          massiv1[h].y:=h;
          massiv1[h].x:=h;
        end;
      if ((i+50) mod 20)and((id+50) mod 20) = 0 then massiv1[h].y:=-massiv1[h].y;
{      if sqrt(sqr(i-50)+sqr(id-50))>50 then massiv1[h].y:=-300;   }
    end;
  end;
  ...
  repeat
    ...
    for i:=1 to 10000 do
      put3dpixel(massiv2[i].x,massiv2[i].y,massiv2[i].z,green);
